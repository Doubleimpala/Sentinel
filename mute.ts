    ...
    callback: async ({
        ...
        try{
            ...
            if(member){
                ...
                if(!muteRole){
                    return "A muted role does not exist. Please create a role named 'Muted' exactly as it is written here and configure it as you would like. \n *This has been done so that server owners can best configure their own roles.*"
                }
            ...
            return `Cannot mute @${userId}`
        }

        const muteConfirmation = new MessageEmbed()
            .setDescription (`<@${userId}> has been muted.`)
